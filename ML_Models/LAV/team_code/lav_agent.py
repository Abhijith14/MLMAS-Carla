...
cwd = os.getcwd()
print(cwd)
models_path = os.path.join(cwd, "ML_Models", "LAV", "lav")
...
models_path = os.path.join(models_path, "utils")
sys.path.append(models_path)
print(models_path)
...
class LAVAgent(AutonomousAgent):
    ...
    def setup(self, path_to_conf_file):
        ...
        with open(path_to_conf_file.replace("team_code/config.yaml", "config.yaml"), 'r') as f:
            # print("f says : ", f.readline())
            config = yaml.safe_load(f)

        print("Here you go : ", config, path_to_conf_file)
